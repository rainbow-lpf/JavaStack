...
## 〇、IoC 与 AOP — 两大基石（前置知识）

| 概念 | 一句话 | 实现手段 |
|------|------|------|
| **IoC 控制反转** | 对象的创建、组装、生命周期管理权，从业务代码反转给容器 | DI 依赖注入（@Autowired / 构造器注入） |
| **AOP 面向切面** | 把日志、事务、权限等横切逻辑抽成切面，运行时织入目标方法 | 动态代理（JDK / CGLIB） |

> 两者的关系：AOP 建立在 IoC 之上——正因对象由容器创建，初始化后才能被偷偷替换成代理对象。
>
> 渐进式详解（含"IoC 和 DI 的区别""为什么 this 调用不走代理"等连环追问）：
> [IOC与AOP-渐进式讲解.md](./IOC与AOP-渐进式讲解.md)
...
> **IoC**：对象的创建和依赖装配从业务代码反转给容器，通过依赖注入实现，换来解耦、可测试、统一生命周期。
> **AOP**：日志、事务这类横切逻辑抽成切面，运行时通过动态代理织入目标方法。AOP 建立在 IoC 之上——正因对象由容器创建，才能被替换成代理。
