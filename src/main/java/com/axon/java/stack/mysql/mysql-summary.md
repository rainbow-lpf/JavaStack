...
| 方案 | 存储 | 精度 | 适用 |
|------|------|------|------|
| **`BIGINT` 存"分"**（大厂标配） | 8 字节整数 | ✅ 完全精确 | 支付/交易核心链路 |
| **`DECIMAL(M,D)`** | 十进制打包（每 9 位 4 字节） | ✅ 精确 | 费率、汇率 |
| `FLOAT` / `DOUBLE` | IEEE 754 二进制浮点 | ❌ **天然不精确** | **禁碰钱** |

```sql
amount_fen BIGINT NOT NULL COMMENT '金额，单位：分'   -- 交易金额
rate      DECIMAL(8,6) COMMENT '费率'                 -- 非整数场景
```

### 精度丢失什么时候发生

**根因：十进制 0.1 在二进制里是无限循环小数**（`0.000110011001100...`），DOUBLE 尾数 52~53 位存不下 → 截断舍入 → 从出生就是近似值：
...
| # | 场景 | 说明 |
|---|------|------|
| 1 | FLOAT/DOUBLE 字段存金额 | 存 19.99 → 实际 19.9899997711... |
| 2 | double 参与运算 | 累加/乘汇率后误差放大 |
| 3 | `new BigDecimal(double)` ⚠️ 最隐蔽 | 把 double 的误差原样带进来，必须 `new BigDecimal("0.1")` 字符串构造 |
| 4 | 隐式类型转换 | DECIMAL 列和 DOUBLE 列计算 → 被拉低成浮点 |
| 5 | JSON 序列化用 double | 前端/下游拿到变形数字 |

```java
// 除法必须指定精度和舍入，否则除不尽直接抛异常
...
```
① 跨语言无歧义：DECIMAL 若某层按 double 映射/序列化 → 精度照丢；整数到哪都精确
② 和支付渠道对齐：微信、支付宝接口金额单位就是"分"
③ 计算简单：无 scale / rounding 之争
```

**最佳实践：交易金额 BIGINT 分 + 费率 DECIMAL + 展示层换算元 + JSON 全程无浮点。**

### 话术（30 秒）

> 金额绝不用 FLOAT/DOUBLE——0.1 在二进制是无限循环小数，IEEE 754 存不下，从存储那一刻就是近似值，运算还会放大误差。交易金额用 BIGINT 存"分"（和支付渠道接口对齐），费率汇率用 DECIMAL（十进制打包存储是精确的）。两个经典坑：BigDecimal 必须字符串构造，除法必须指定 scale 和 RoundingMode。JSON 传输也避免 double，保证从库到前端全程无浮点。
